import { NavModule, UserRole } from '@/types/uniafy';

export const NAV_MODULES: NavModule[] = [
  {
    id: 'mestre',
    label: 'Master',
    icon: 'Crown',
    roles: [UserRole.MASTER],
    items: [
      {
        id: 'gestao-agencias',
        title: 'Assinaturas & Financeiro',
        description: 'Controle de planos, faturamento e inadimplência',
        icon: 'Building2',
        path: '/mestre/agencias',
        roles: [UserRole.MASTER],
      },
      {
        id: 'config',
        title: 'Central de Alertas',
        description: 'Automação de notificações e réguas de cobrança',
        icon: 'Settings',
        path: '/mestre/config',
        roles: [UserRole.MASTER],
      },
      {
        id: 'cofre',
        title: 'Cofre API',
        description: 'Chaves seguras (Stripe, Asaas)',
        icon: 'Key',
        path: '/mestre/cofre',
        roles: [UserRole.MASTER],
      },
      {
        id: 'banco-sql',
        title: 'Banco SQL',
        description: 'Execução direta de comandos no banco de dados',
        icon: 'Database',
        path: '/mestre/sql',
        roles: [UserRole.MASTER],
      },
      // --- SAAS SUITE ---
      {
        id: 'audit-logs',
        title: 'Logs de Auditoria',
        description: 'Rastreabilidade e segurança',
        icon: 'ShieldAlert',
        path: '/mestre/audit',
        roles: [UserRole.MASTER],
      },
      {
        id: 'product-analytics',
        title: 'Product Analytics',
        description: 'Telemetria de uso e engajamento',
        icon: 'Activity',
        path: '/mestre/analytics',
        roles: [UserRole.MASTER],
      },
      {
        id: 'saas-metrics',
        title: 'Métricas SaaS',
        description: 'Inteligência financeira (MRR, LTV)',
        icon: 'TrendingUp',
        path: '/mestre/metrics',
        roles: [UserRole.MASTER],
      },
      {
        id: 'white-label',
        title: 'Fábrica White Label',
        description: 'Domínios e Branding Personalizado',
        icon: 'Globe',
        path: '/mestre/whitelabel',
        roles: [UserRole.MASTER],
      },
      {
        id: 'gestao-planos',
        title: 'Gestão de Planos',
        description: 'Configuração de planos e preços',
        icon: 'CreditCard',
        path: '/mestre/planos',
        roles: [UserRole.MASTER],
      },
      {
        id: 'branding',
        title: 'Identidade Visual',
        description: 'Personalização global de cores e logo',
        icon: 'Palette',
        path: '/mestre/branding',
        roles: [UserRole.MASTER],
      },
    ],
  },
  {
    id: 'growth',
    label: 'Growth',
    icon: 'Target',
    roles: [UserRole.MASTER, UserRole.AGENCY],
    items: [
      {
        id: 'g-hunter',
        title: 'G-Hunter Scraper',
        description: 'Captação ativa de leads via Google Maps',
        icon: 'Search',
        path: '/growth/hunter',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      },
      {
        id: 'cnpj-sniper',
        title: 'CNPJ Sniper',
        description: 'Extração industrial de dados da Receita Federal',
        icon: 'FileText',
        path: '/growth/cnpj',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      },
      {
        id: 'crm-prospect',
        title: 'Máquina de Vendas',
        description: 'CRM Kanban de prospecção estratégica',
        icon: 'Target',
        path: '/growth/crm',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      }
    ],
  },
  {
    id: 'agency',
    label: 'Agência',
    icon: 'Building2',
    roles: [UserRole.MASTER, UserRole.AGENCY],
    items: [
      {
        id: 'agency-users',
        title: 'Time & Acesso',
        description: 'Gestão de equipe e permissões',
        icon: 'Users',
        path: '/agency/users',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      },
      {
        id: 'agency-whitelabel',
        title: 'Personalização (White Label)',
        description: 'Domínio personalizado e branding',
        icon: 'Globe',
        path: '/agency/whitelabel',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      },
      {
        id: 'agency-clients',
        title: 'Clientes',
        description: 'CRM e integração de contas',
        icon: 'Briefcase',
        path: '/agency/clients',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      },
      {
        id: 'onboarding-magico',
        title: 'Onboarding',
        description: 'Setup automático de novos clientes',
        icon: 'Zap',
        path: '/agency/onboarding',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      },
      {
        id: 'gestao-squads',
        title: 'Squads',
        description: 'Distribuição de clientes e tarefas por equipe',
        icon: 'LayoutGrid',
        path: '/agency/squads',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      },
      {
        id: 'agency-integrations',
        title: 'Integrações',
        description: 'Conexão com plataformas (Meta, Google)',
        icon: 'Link',
        path: '/agency/integrations',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      },
      {
        id: 'agency-contracts',
        title: 'Contratos',
        description: 'Gestão de contratos e assinaturas',
        icon: 'FileSignature',
        path: '/agency/contracts',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      },
      {
        id: 'agency-finance',
        title: 'Financeiro',
        description: 'Controle de faturamento e dashboards',
        icon: 'CreditCard',
        path: '/agency/finance',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      },
      {
        id: 'churn-alert',
        title: 'Radar de Churn',
        description: 'Alertas preditivos de risco',
        icon: 'Siren',
        path: '/agency/churn-alert',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      }
    ],
  },
  {
    id: 'traffic',
    label: 'Tráfego',
    icon: 'Zap',
    roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
    items: [
      {
        id: 'traffic-dashboard',
        title: 'Painel Central',
        description: 'Visão 360º de todas as métricas',
        icon: 'LayoutDashboard',
        path: '/traffic/dashboard',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'sentinela',
        title: 'Sentinela de Grupos',
        description: 'Monitoramento 24/7 de anomalias',
        icon: 'ShieldAlert',
        path: '/traffic/sentinela',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'traffic-guard',
        title: 'Guardião (Automação)',
        description: 'Regras de stop-loss e escala',
        icon: 'ShieldCheck',
        path: '/traffic/guard',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'lp-sentinel',
        title: 'LP Sentinel (Big Brother)',
        description: 'Monitoramento de uptime e velocidade',
        icon: 'Activity',
        path: '/traffic/lp-sentinel',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'creative-hub',
        title: 'Central de Criativos',
        description: 'Biblioteca e aprovação de ads',
        icon: 'Image',
        path: '/traffic/creatives',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'ad-spy',
        title: 'Ad Spy & Referências',
        description: 'Espionagem e banco de ideias',
        icon: 'Eye',
        path: '/traffic/ad-spy',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'tracking-manager',
        title: 'Tracking & UTMs',
        description: 'Construtor de UTMs e pixels',
        icon: 'Target',
        path: '/traffic/tracking',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'media-planner',
        title: 'Planejador de Mídia',
        description: 'Simulador de budget e resultados',
        icon: 'PieChart',
        path: '/traffic/planner',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'budget-pacing',
        title: 'Controle de Ritmo',
        description: 'Velocímetro de gasto vs meta',
        icon: 'Gauge', // Precisaremos mapear se não existir, ou usar Timer/Speedometer
        path: '/traffic/pacing',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'offline-conversions',
        title: 'Conversões Offline',
        description: 'ROAS Real (Upload de Vendas)',
        icon: 'UploadCloud', // Precisaremos mapear
        path: '/traffic/offline',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'audience-vault',
        title: 'Cofre de Públicos',
        description: 'Biblioteca de personas e listas',
        icon: 'Users',
        path: '/traffic/audiences',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'naming-governor',
        title: 'Governador de Nomes',
        description: 'Padronização obrigatória de campanhas',
        icon: 'Type', // Precisaremos mapear
        path: '/traffic/naming',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'criador-campanhas',
        title: 'Criador de Campanhas',
        description: 'Setup rápido de ads multi-plataforma',
        icon: 'Zap',
        path: '/traffic/campaigns',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'relatorios-dinamicos',
        title: 'Relatórios Dinâmicos',
        description: 'Builder drag-and-drop de reports',
        icon: 'FileText',
        path: '/traffic/reports',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'analise-ia',
        title: 'Análise IA (Especialistas)',
        description: 'Insights estratégicos via Inteligência Artificial',
        icon: 'Brain',
        path: '/traffic/ai-analysis',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'agenda-operacional',
        title: 'Agenda Operacional',
        description: 'Cronograma de otimizações e tarefas',
        icon: 'Calendar',
        path: '/traffic/scheduler',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'analise-dados',
        title: 'Análises de Dados',
        description: 'Ferramentas avançadas de Big Data',
        icon: 'LineChart',
        path: '/traffic/data-analysis',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'lembretes',
        title: 'Lembretes',
        description: 'Central de notificações e follow-ups',
        icon: 'Bell',
        path: '/traffic/reminders',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      }
    ],
  },
  {
    id: 'success',
    label: 'Clientes',
    icon: 'Users',
    roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
    items: [
      {
        id: 'portal-cliente',
        title: 'Portal White Label',
        description: 'Experiência exclusiva para o cliente final',
        icon: 'Globe',
        path: '/success/portal',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'aprovacao-criativos',
        title: 'Central de Aprovação',
        description: 'Workflow de aprovação de anúncios',
        icon: 'CheckCircle2',
        path: '/success/aprovacao',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'health-score',
        title: 'Health Score',
        description: 'Métrica de satisfação e risco de churn',
        icon: 'HeartPulse',
        path: '/success/health',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      }
    ],
  },
  {
    id: 'sistema',
    label: 'Sistema',
    icon: 'Settings',
    roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
    items: [
      {
        id: 'meu-perfil',
        title: 'Meu Perfil',
        description: 'Dados de acesso e segurança',
        icon: 'User',
        path: '/sistema/perfil',
        roles: [UserRole.MASTER, UserRole.AGENCY, UserRole.CLIENT],
      },
      {
        id: 'conexoes-nuvem',
        title: 'Conexões & APIs',
        description: 'Hub de integrações e webhooks (n8n)',
        icon: 'Share2',
        path: '/sistema/conexoes',
        roles: [UserRole.MASTER, UserRole.AGENCY],
      }
    ],
  },
];
