...
SELECT 'Bucket Identidade Visual configurado e permissões corrigidas!' as status;`,
        icon: <ShieldCheck className="w-4 h-4 text-green-500" />,
        category: 'setup'
    },
    {
        id: 'fix-user-profile-storage',
        title: '[STORAGE] Corrigir Bucket Imagem Perfil (Upload)',
        query: `-- SCRIPT PARA CORRIGIR PERMISSÕES DO BUCKET DE PERFIL
-- Garante que o bucket 'Imagem Perfil Usuarios' exista e permite upload

-- 1. Criar Bucket se não existir
INSERT INTO storage.buckets (id, name, public)
VALUES ('Imagem Perfil Usuarios', 'Imagem Perfil Usuarios', true)
ON CONFLICT (id) DO UPDATE SET public = true;

-- 2. Limpar políticas antigas deste bucket
DROP POLICY IF EXISTS "Public Profile Images" ON storage.objects;
DROP POLICY IF EXISTS "User Upload Profile Image" ON storage.objects;
DROP POLICY IF EXISTS "User Update Profile Image" ON storage.objects;
DROP POLICY IF EXISTS "User Delete Profile Image" ON storage.objects;

-- 3. Criar Políticas

-- Leitura Pública (Avatar visível para todos)
CREATE POLICY "Public Profile Images"
ON storage.objects FOR SELECT
USING (bucket_id = 'Imagem Perfil Usuarios');

-- Upload Autenticado
CREATE POLICY "User Upload Profile Image"
ON storage.objects FOR INSERT
TO authenticated
WITH CHECK (bucket_id = 'Imagem Perfil Usuarios');

-- Update Autenticado
CREATE POLICY "User Update Profile Image"
ON storage.objects FOR UPDATE
TO authenticated
USING (bucket_id = 'Imagem Perfil Usuarios');

-- Delete Autenticado (para substituir foto)
CREATE POLICY "User Delete Profile Image"
ON storage.objects FOR DELETE
TO authenticated
USING (bucket_id = 'Imagem Perfil Usuarios');

SELECT 'Bucket Imagem Perfil Usuarios configurado com sucesso!' as status;`,
